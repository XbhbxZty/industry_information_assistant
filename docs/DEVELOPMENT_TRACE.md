...
## 2026-09-03：学习手册十二章扩展检查点

- **范围**：按用户“继续补齐其他章节”的要求，在原档案样章基础上补齐其余十一章；只改文档及其生成/验证脚本，不实现或修改业务功能。
- **产物**：同一个离线 `docs/project-handbook/index.html`；十二章正文、逐层子过程、可点击过程图/对照表、三十四个词条及一百三十余处固定版本源码（准确数量见生成输出）。旧章节、样章、术语和源码行 ID 保留，并增加回归检查。
- **维护方式**：五组章节 HTML 与对应 refs JSON 分开维护，构建为单文件；原样章与交互仍在模板，不重做已验证部分。业务源码基线保持 `f5f8d17`，旧链接合同以 `7b4c66e` 的交付文件为准。
- **分工**：主代理撰写 01–02、10–12，集成、核对和收尾；三个 Terra/high 子代理分别撰写并交叉复核数据源/RAG、多 Agent/风险、存储/人工复核。
- **事实边界**：独立离线案例 RAG 未接主图；管理员材料仍为档案内关键词检索；场景项不混入核心评分；调查层含可选 LLM 探索但不改裁决；普通读取与复核工作台的失败语义有差异。所有这些均按当前源码说明，没有为了使讲解成立而改业务。
- **验证**：文档十二组静态/纯逻辑检查及可复现生成通过；最终执行结果和文档提交见下方补记。没有启动业务服务、数据库迁移、外部模型或网络检索，也未重跑历史 102 项后端/8 项前端业务验收；未做浏览器视觉/真实 DOM 验收。
- **下次入口**：直接打开/刷新 `index.html`；从具体章节或源码锚点继续加深。若改业务基线，重新核对文字、数字口径、源码范围和历史测试证据，不自动跟随 HEAD。

### DEV-BC-20260903-034：文档把独立组件、探索产物与裁决路径概括为同一流程

- **阶段 / 状态**：学习手册十二章扩展 / 已关闭（文档事实修正）
- **发现方式**：章节交叉复核和主代理对照调用点、评分完整度函数与调查层实现。
- **现象 / 根因 / 影响**：初稿暗示离线案例检索已走主研究证据桥，过度限定最终报告只能含确定性产物，并未突出场景项不参与核心评分。把“组件存在”“未来应有的接线”和“当前运行路径”混在一起，会让读者误解实现进度。
- **解决办法**：明确 case corpus 仅见定义/测试调用，并补真实 case loader 子过程；说明报告可以包含独立调查层 LLM 发现，但裁决结论与核心评分不被它改写；场景项与核心二十项分别解释。补足 Scout 统一收口、Critic 达上限仍需人审、批准且改级才重算建议等实际分支。
- **回归保护 / 验证**：章节正文直接链接固定基线唯一锚点；三个作者交叉复核、主审逐项修订。所有文件仅属手册，不把发现当作已修复业务缺陷。
- **提交**：见本节最终提交补记。
- **遗留风险**：组织级 RAG 和离线组件主流程接线仍未实现；本文不新增实现承诺。

### DEV-BC-20260903-035：文档把局部完整性和错误策略写成全系统保证

- **阶段 / 状态**：学习手册十二章扩展 / 已关闭（文档事实修正）
- **发现方式**：复核实际异常捕获、序列化边界、全局 checkpointer 和签名投影函数。
- **现象 / 根因 / 影响**：初稿把个人 KB 的细分失败原因描述为会传给 Scout；把普通 checkpoint 读取写得与严格复核接口相同；沿用日志暗示 MemorySaver 不能跨请求；没有区分独立 UI/报告列与 state_json 的 MAC 覆盖范围。读者可能据此错误判断数据安全或恢复能力。
- **解决办法**：说明路由只日志记录细分原因，再传空 scope；completed 文档的 Milvus 读取异常目前返回空 chunks；普通 checkpoint 非完整性异常可日志后返回 None，复核工作台另有严格读取；全局 MemorySaver 可复用于同进程请求，但不保证跨进程/重启；同事务写入不等于每列都直接签入业务 MAC。另明确 claim token 仅由服务端管理、客户端按同身份和原决定重试。
- **回归保护 / 验证**：补实际读取、上下文签名和提交路径引用，来源片段唯一性与文内链接检查通过；推演反例明确标为推演，不冒充本轮实测事故。
- **提交**：见本节最终提交补记。
- **遗留风险**：上述兼容性降级和错误细节丢失是现有代码边界，未在文档任务中修改。

### DEV-BC-20260903-036：章节扩展后原固定数量断言及旧链接检查出现测试设施误报

- **阶段 / 状态**：学习手册十二章扩展 / 已关闭（文档验证脚本）
- **发现方式**：实际运行验证先报 `132 !== 37`；新增旧链接回归又报 `removed sample target handbook-script`，但原 script 元素仍在文件中。
- **现象 / 根因 / 影响**：原验证只允许样章的 37 个来源；新链接检查拿完整旧 HTML 的 ID 与剔除 script/style 后的新集合比较，比较范围不一致。两者均是检查设施错误，不能为此删掉新增来源或断言已经丢失旧页面内容。
- **解决办法**：来源数量从五组显式 refs 清单计算，仍逐一断言源码 ID 存在；原 37 个来源保持。旧版 ID 回归改为两侧完整 HTML 的 ID 集合比较；链接完整性、唯一性、脚本语法等原检查继续保留，新增十二章正文与三十四个词条合同。
- **回归保护 / 验证**：修订后十二组文档检查通过，包含所有旧版 ID、全部文内链接与固定基线生成一致性。新引用定位仍要求唯一命中，未放宽为选第一个结果。
- **提交**：见本节最终提交补记。
- **遗留风险**：静态解析器针对本手册的显式标记，不是任意 HTML5 解析器；通过不代表视觉和真实 DOM 已验收。

### 十二章最终提交补记

- **文档提交**：`0f0a317`（同时包含本轮 034–036 的说明与修正）。
- **最终结果**：独立 HTML 2,312,753 字节、十二章正文、34 个术语、136 个源码快照；`node docs/project-handbook/build.mjs`、`node docs/project-handbook/verify.mjs`（12 组）、`git diff --check` 通过。旧版所有 ID 保留，生成仍锚定 `f5f8d17`。
- **边界不变**：未做浏览器视觉/真实 DOM 验收，未重跑业务验收，未改业务代码。后续可直接从此文档检查点继续，无需重做已完成章节。
