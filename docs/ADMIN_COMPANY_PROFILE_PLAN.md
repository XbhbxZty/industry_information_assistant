...
> 3.4D4b1 领取/接受决定、D4b2a 原子终稿与写入保护、D4b2b HTTP/SSE 与图恢复已完成；
> 3.5 无浏览器开发功能验收已通过，代码检查点 `f4c19c7`；浏览器交互按用户确认单列待补。
> 修改前测试入口已保存为 `f81cfef`；既有业务基线 `0805b4a` 未重新实现。
...
| 3.4D4b | reviewer 原子领取、幂等裁决与断流恢复 | 已完成（开发版，浏览器交互另补） | `fc707fb`、`42a00fe`、`0805b4a` |
| 3.5 | PostgreSQL、独立进程和前端契约验收 | 已通过（开发版）；浏览器交互待补 | `f4c19c7` |

### 2026-09-02 起的执行口径（用户已确认）

优先真实实现和开发环境闭环；已经完成的协议/测试保留，不重新实现。剩余工作按五批交付：
D2a2b＋D2a3 → D2b → D3 最小轮换 → D4a＋D4b → 3.5 集中联调（现均已按开发口径完成）。
...
**D4b2b 原接续要求（现已实现，交付记录如下）**：在现有 POST 的流建立之前领取/接受决定；图恢复使用已保存的决定和
...
#### 2026-09-02 D4b2b 交付与恢复点

代码检查点 `0805b4a`，开始前为 `c266b98`（工作区干净）。一个 Terra/high 子代理实现图接线，
主代理完成 HTTP/短事务服务集成、前端错误提示、真实 PostgreSQL 测试、复核修正与提交。
复用已验证的 claim/accept/finalize，不改迁移，head 仍为 `20260902_0004`。

- 现有复核按钮使用原 POST 格式。服务端先实际查库确认 reviewer，再以单个短事务领取＋接受
  决定；无效决定回滚领取。身份与接受时间由服务器写入，token 不进入前端或图状态。
- SSE 打开后再次读取已接受记录；同人同决定可重试，不允许替换已接受决定或抢走其他人的
  accepted。完成态重试只回放已验证持久结果，不重新运行图。
- 暂停图先验签并严格匹配冻结材料，才恢复 human_review；完成图必须匹配冻结材料＋原决定
  的确定性输出。真正缺失图时只补做最后的人审变换，不重新检索/分析。损坏和漂移不能冒充缺失。
- 初次暂停先保存最新完整封签状态，再置 paused。claimed 执行不走普通检查点写入，终局只走
  原子 finalizer；两类完成事件均在提交成功后发出。流关闭/失败不写 failed、不释放 accepted。
- 前端解析流建立前的 JSON 错误，普通 409 不再误报材料损坏；断流提示使用原决定重试。

最终关联组合 **`343 passed`，无跳过**（2966 条 warnings，未扩展为告警治理），含新增
**35 项真实 PostgreSQL＋HTTP/图测试**：批准/驳回/改判、标准/管理档案、同决定重试、DB 身份
与严格输入、提交前回滚/提交后响应丢失、双 reviewer HTTP 竞争、双编译图实例竞争、坏签名与
合法签名内容漂移、缺失图恢复、两个事件断点、提前完成事件过滤。旧 claim/finalizer/写保护、
检查点/密钥/迁移、人审/访问/工作台/图恢复等也在组合内通过。

前端 `node --test tests/review-response.test.mjs` **5 passed**；变更文件 ESLint、`npm run build`、
后端编译和 diff check 通过。单独全前端类型检查并未通过：同依赖下 `c266b98` 基线 25 项，
本版 24 项，新增诊断 0；仅修复当前复核页 nullable 字段错误，其余旧类型债记录延期。

测试使用真实随机 PostgreSQL 库与实际 FastAPI/TestClient、LangGraph MemorySaver；模型/搜索
代理与身份令牌解码是替身，但 reviewer 最终授权查实际 DB。双图竞争是同进程两编译实例，
流关闭是生成器事件断点，**不等同独立后端进程或真实浏览器网络断流验收**。未运行后端全量。
随机测试库已确认清理；未修改已有业务数据库、用户 `.env` 或运行中容器，未推送远端。
Bad Case `DEV-BC-20260902-023`～`026` 已关闭；`027` 的旧模块类型债已登记。

**D4b2b 当时的接续要求（验收口径已按下文 3.5 更新）**：先核对 Git 检查点与本地服务状态，使用隔离测试库及测试账号启动
独立后端，再在浏览器串起管理员建档→普通用户发起→版本冻结→独立复核→报告归档。
补断流重试、错误提示、权限与个人知识库隔离矩阵，只修阻塞演示闭环的明确问题。不重复本轮
已验证实现，不自动扩展全前端类型治理；真实业务库升级仍须先确认目标、停写与备份。
...
**用户确认的新验收口径**：Browser 连接暂不可用，不再阻塞开发功能验收；本轮不再尝试其他 UI
自动化。真实浏览器交互单列待补，不能将 HTTP 测试表述为已完成页面端到端验收。

使用真实 PostgreSQL、独立 Uvicorn 进程、真实登录/JWT 与 HTTP/SSE 覆盖管理员建档、普通用户
发起、版本冻结、独立复核、改判留痕、归档、篡改拒绝、权限和个人知识库隔离。特别验证图使用
PostgreSQL 保存，以及真实断开连接/终止进程/重启后，原决定与终稿仍唯一且可恢复。
模型/外部搜索使用确定性输入；权限、档案、规则评分、图和数据库服务不替身。

前端复用实际请求模块验证响应契约，并进行相关测试、构建和变更文件 lint。最终只能标为
“开发功能闭环验收通过，浏览器交互待补测”，不保证页面布局、按钮、焦点和表单交互已实测。
手工补测清单：登录 → 管理员建档/修改 → 普通用户选档发起 → 独立复核 → 报告读取；另补
无权限入口、无效输入提示、断流后重试和页面切换。无需 Browser 插件也可由用户手工执行。
生产部署和真实模型报告质量另行验收。

#### 3.5 可重跑的验收入口

- `backend/tests/test_process_acceptance_postgres.py`：四个真实进程用例。管理员建档/修订/归档；
  普通用户发起与版本冻结；独立复核改判、幂等回读；两个真实 TCP 断流/kill/restart 时点；
  检查点与档案篡改拒绝。真实密码登录/JWT、API、规则评分、PG 图检查点和最终事务不替身。
- `backend/tests/test_e2e_entry_contract.py`：测试设施的纯合同验证。本地数据库 URL/随机库边界、
  query 覆盖拒绝、路由 Depends 引用及 Scout 构造前隔离，防止准备入口静态编译通过但实际失效。
- `frontend/tests/company-profile-response.test.mjs`：档案资源状态与通用 envelope 分离；保留失败
  HTTP 原异常；仅详情/创建/修改/归档启用资源状态标记。复用实际源码，不新增浏览器框架。

复跑后端：在 backend 目录为当前进程显式设置本地 `MIGRATION_TEST_ADMIN_URL`，运行
`python -m pytest -q tests/test_process_acceptance_postgres.py tests/test_e2e_entry_contract.py`。
该账号须能创建/清理随机测试库并读取集群身份；未设置时真实 PG 用例会跳过，不能把跳过当验收。
每次只对自建随机库迁移/造数，退出时校验库名/OID/owner/集群身份后删除；不升级已有业务库。
子进程只继承必要系统环境，签名/JWT 为新生成测试值，不加载 `.env`，不调用外部模型或搜索。

复跑前端：在 frontend 目录运行
`node --test tests/company-profile-response.test.mjs tests/review-response.test.mjs`，再执行
`npm run build`。完整 TypeScript 检查另用 `tsc --noEmit --project tsconfig.app.json`，既有
24 项旧诊断不在本阶段扩大治理。

个人知识库隔离在本轮验证的是关系库存储不增加/不混入、用户列表隔离与 reviewer 最小投影；
不宣称真实 Milvus 召回、文档解析或模型报告质量已验收。材料检索仍为已有档案级受控检索，
组织级向量材料库是下节后续方向，未在这次验收中顺带实现。

#### 2026-09-02 3.5 交付与恢复点

代码检查点 **`f4c19c7`**。修改前先保存中断遗留测试入口为 `f81cfef`。两个 Terra/high 子代理
分别负责测试入口审查/隔离修正、前端请求合同与修复；主代理负责独立进程用例、真实运行、
集成复核、修正测试夹具与类型回归、追踪台账和 Git 检查点。未修改生产后端或迁移。

最终关联组合 **102 passed，无跳过**（1048 条 warnings，未扩大为告警治理）：
`test_process_acceptance_postgres.py`、`test_e2e_entry_contract.py`、
`test_review_submission_postgres.py`、`test_admin_company_profiles.py`、
`test_research_access.py`、`test_review_workspace.py`。
新增的四项独立进程验收及十一项入口合同测试均在组合内。两处故障实际关闭 TCP 并终止自己
创建的后端进程，在同一随机 PG 库上重启，证明 accepted/paused 与 finalized/completed
均能回读原决定/接受时间，最终业务版本只增加一次；不是把进程内 mock 称为进程恢复。

前端 **8 项 Node 行为测试通过**，变更 TS 文件 lint、`npm run build`、后端编译与
`git diff --check` 通过。独立类型检查仍有 24 项旧诊断；同依赖读取 `f81cfef` 三份变更前文件
作内存基线也是 24 项，新增 0。原有构建大包告警保留。未跑全后端、浏览器或真实模型质量评测。

实际修复的产品问题：档案 active/archived 状态被通用前端拦截器误当 API 错误，已限定四个
资源接口修正；HTTP 403/409/422 仍保持失败。Bad Case `DEV-BC-20260902-028`～`030` 已记录
复现、根因、修复和回归，其中入口/客户端/造数/故障 SQL 属于测试设施问题，不混称业务缺陷。

随机测试数据库已只读确认清理；子后端进程均由夹具停止。未修改已有业务库、用户 `.env`
或运行中容器，未推送远端。真正使用本版前，现有业务库仍须按已完成的迁移流程确认目标、
备份和停写后升级至 `20260902_0004`；本轮没有代为执行，Git 回退也不等于数据库回退。

**结论与下一步**：管理员档案专项已达到“开发功能闭环验收通过，浏览器交互待补测”。下次先
检查 Git 状态与本节记录，不重做这些实现或验收；可继续组织级材料/RAG 的下一批计划。
页面人工补测和真实环境初始化独立安排，不再混入当前已完成检查点。
