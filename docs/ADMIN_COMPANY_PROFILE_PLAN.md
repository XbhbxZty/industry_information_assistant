...
> 3.4D4b1 领取/接受决定的短事务服务已完成；下一步为 D4b2 最终落盘和 SSE。
...
| 3.4D4b | reviewer 原子领取、幂等裁决与断流恢复 | D4b1 已完成；D4b2 待开始 | `fc707fb`（D4b1） |
...
**下次从 D4b2 开工**：以本检查点为基线，先接有效 claim 的普通写入保护与唯一最终事务，
再接现有 POST/图恢复和 SSE 完成事件；重复请求、已接受未完成恢复与断流/双图竞争一起验证。
不重做已验证的 D4b1；D4b2 全链路接线前，不对旧流程手工调用新领取服务。
