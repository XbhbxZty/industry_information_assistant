...
目前只能由确定性测试证明其工程属性：显式违规 7/7、对照误报 0/18、
零 token、毫秒级、可进 CI、LLM 不可用时照常工作。
它对系统级 LLM 检出率和方差的增量必须等待 v3 实验，不能沿用 3.3% → 0%。
...
- **开发集过拟合**：这 28 例已反复用于提示词调优、架构修改与模型选型。
  ...
  需新增未参与任何调优的独立留出集。
- A/B/C/D 组均需按 v3 口径重跑才能纳入同一张表。
- 已建立 `critic_holdout.json`（6 注入 + 8 对照）；首次完整运行后冻结 SHA256，
  不参与任何提示词或扫描词表调优。它仍只是 case-level holdout，不是企业分布留出。
