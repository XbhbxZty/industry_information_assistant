...
## 一·五、消融实验（⚠️ 部分结论已撤回，实验设计有缺陷）

> 2026-08-10 实测。触发问题：换用更强模型后误报大幅下降，
> 是否意味着此前的架构工作被模型能力溢出？
>
> **本节保留原始数据与已发现的方法论缺陷。带 ⚠️ 的结论不可引用。**

### 原始数据（28 例 × 3 次）

| 配置 | 检出稳定 | 检出逐次 | 对照组无误报稳定 | 无误报逐次 |
|---|---|---|---|---|
| A 清单+扫描器合并+v4-flash | 10/10 | 100% | 17/18 | 98.1% |
| ⚠️ B 清单+扫描器**未真正关闭** | 8/10 | 93.3% | 17/18 | 96.3% |
| C 无清单 + v4-flash | 3/10 | **56.7%** | 17/18 | 98.1% |
| D 清单+扫描器合并+v3.2 | 10/10 | 100% | 9/18 | **72.2%** |

### ⚠️ 已撤回的结论

**撤回 1：「扫描器贡献 +6.7 个百分点」**
B 组的 `--ablate scanner` 只绕过了 `merge_review()` 的结果合并，
但 `_review_content()` **自身仍调用 `scan_report()` 并把扫描结论写入提示词**。
B 组的 LLM 依然看得到扫描结果，因此 A/B 差异只反映"扫描结论是否并入最终 issues"，
**不构成扫描器的独立贡献**。真正的消融需同时关闭：扫描执行、提示词注入、
结果合并、类型过滤。该数字作废，待重做。

**撤回 2：「架构与模型完全正交，各自只影响一个维度」**
样本仅 10 注入 + 18 对照，且这套用例已被反复用于提示词调优、架构修改与模型选型，
存在**开发集过拟合**。现有数据最多支持趋势性表述，不足以证明"完全正交"或"必然如此"。

### 目前可信的结论（措辞已收敛）

1. **结构化清单为模型提供了正文中无法恢复的核查状态。**
   C 组去掉清单后检出逐次率 100% → 56.7%，这一差异幅度大且机制清晰：
   「已查询但无记录」与「未查询」的区分只存在于 `coverage.queried` 字段，
   报告正文与原始事实里都没有这个 bit。
2. **更强的 Critic 模型在当前评测集上显著降低了清单类误报**
   （逐次无误报率 72.2% → 98.1%，D 组 vs A 组）。
3. **扫描器在当前 7 个显式违规用例上提供零方差兜底**
   （`eval/run_scanner.py` 独立测得 7/7 检出、0/18 误报）。
   其价值是工程属性——零 token、毫秒级、可进 CI、LLM 不可用时照常工作——
   **而非已被证实的准确率增益**。

### 指标口径更正

此前称"误报率 98.1%"是**错误命名**。98.1% 是**逐次无误报率**，
对应误报率为 **1.9%**。且 `_false_positive()` 仅统计
`unverified_as_fact` / `conflict_silently_resolved` / `unsupported_risk_conclusion`
三种类型——若模型把错误意见标为 `hallucination`、`logic_error` 或返回字符串形态，
均不计入。因此该数字**不能解释为"整体误报很低"**。

### 待补的实验条件

- 真正的扫描器消融（四处同时关闭）
- **独立留出集**：未参与任何调优的用例，用于检验开发集过拟合程度
- 误报统计覆盖全部 issue 类型，而非仅三类
- 保存逐次 JSONL 原始产物（输出、耗时、token、错误、模型响应），确保可复算可审计
