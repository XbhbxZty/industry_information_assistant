...
## 一·五、消融实验：架构与模型各自贡献了什么

> 2026-08-10 实测。触发问题：换用更强模型后误报率达 98%，
> 是否意味着此前的架构工作被模型能力溢出、其实可有可无？

### 实验设计
28 例（10 注入 + 18 对照）× 3 次重复，逐项控制单一变量。

| 配置 | 检出稳定 | 检出逐次 | 误报稳定 | 误报逐次 |
|---|---|---|---|---|
| **A 完整架构**（清单+扫描器+v4-flash） | **10/10** | **100%** | 17/18 | 98.1% |
| B 有清单·无扫描器 | 8/10 | 93.3% | 17/18 | 96.3% |
| C 无清单·无扫描器 | 3/10 | **56.7%** | 17/18 | 98.1% |
| D 完整架构 + 弱模型(v3.2) | 10/10 | 100% | 9/18 | **72.2%** |

### 结论：架构与模型正交，各解决一个维度

```
清单驱动设计   检出 56.7% → 93.3%   (+36.6)   决定性
确定性扫描器   检出 93.3% → 100%    (+6.7)    补齐最后一段
换用推理模型   误报 72.2% → 98.1%   (+25.9)   只作用于误报
```

**检出率只随架构变化**（换模型 100%→100%，去清单 100%→56.7%）；
**误报率只随模型变化**（换模型 72.2%→98.1%，去清单 98.1%→98.1%）。
两者没有重叠。

### 为什么必然如此（信息论层面）

以 INJ-01 为例，报告写"经全面核查，该公司近三年无任何涉诉、被执行及失信记录"，
而事实库中不含任何司法条目。模型面对两种解释：

- **查了，确实没有** → 该表述**正确**
- **根本没查**（司法源超时）→ 该表述**严重违规**

**这两种解释在文本层面无法区分。** 该区分只存在于 `coverage.queried`
这个结构化字段中。再强的模型也无法从不存在的信息中恢复信息——
这不是能力问题，是输入里根本没有那个 bit。

而这恰是尽调最危险的失效模式：司法源故障 → 无扣分 → 判低风险 → 放款。

### 对扫描器的诚实评价

+6.7 个百分点，就准确率而言是三项中最边缘的。**它买到的不是准确率，是工程属性**：

| | 扫描器 | LLM |
|---|---|---|
| 方差 | 零（正则） | B 组仍有 2 例不稳定 |
| 成本 | 零 token | 每次调用都花 |
| 速度 | 毫秒 | 数秒 |
| 可进 CI | 是 | 否（慢且贵） |
| LLM 不可用时 | 照常工作 | — |

B 组的 2 例不稳定（INJ-03、INJ-09）说明：无扫描器时**显式违规的检出会时好时坏**。
尽调场景中"大部分时候能抓到把未核实写成无记录"是不够的，这类违规必须每次都抓到。

**因此扫描器保留，但其价值主张应如实表述为"把确定性还给确定性的部分"，
而非"提升了准确率"。**

### 附带发现：结构化输入还在锚定输出形状

C 组首次运行直接崩溃：`AttributeError: 'str' object has no attribute 'get'`。
去掉结构化清单后，模型开始返回 `"issues": ["报告存在幻觉", ...]`——
字符串数组而非对象数组。完整架构下数十轮从未出现。

**结构化输入不止提供信息，也在约束输出结构。** 这一点在准确率指标中不可见——
