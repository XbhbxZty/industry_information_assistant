...
#### 1.5.1 P0 补充的两类（对照外部案例包发现的缺口）

拿一份外部生成的尽调案例包（`case_01` 宁德时代）的 `self_check` 六条与本系统
逐条对照，两条完全没有对应机制：
...
| `no_record_found_misrepresented_as_no_risk` | ✅ `absence_meaningful` + Critic 纪律第 5 条 |
| `related_party_risk_misattributed_to_subject` | ❌ → 新增 `subject_attribution_error` |
| `post_cutoff_information_used_in_cutoff_assessment` | ❌ → 新增 `post_cutoff_evidence` |
| `inaccessible_link_used_as_positive_evidence` | 🟡 适配器路径有、检索路径没有 → BC-51 |

**`subject_attribution_error`（主体归属错误，critical）**

尽调档案是扁平的单主体结构，但现实中的负面事实分属不同法人。案例包给了
教科书级样板：集团担保余额 692.98 亿，明确标注"不得自动归于上市主体单独债务"。

四类违规形态写进了 prompt：集团合并口径当借款人自身债务、子公司失信写成
"该企业失信"、母公司资质当借款主体经营能力佐证、把借款主体义务说成"由集团承担"。

判定要点不是措辞是否精确，而是**这条负面到底是谁的**。允许写
"其控股子公司 X 存在被执行记录，需评估传导影响"，不允许写成借款人自己的记录。

**`post_cutoff_evidence`（越过研究截止日，critical）**

依托第七节的截止日机制。**这一段随截止日开关**：不设截止日时整段不注入。
理由与 BC-18 同形且更严重——让模型去找一个本次根本不存在的问题类型，
它不只是无效，还会主动把正常的时间表述报成越界。

两类都进 `DD_BLOCKING_ISSUE_TYPES`（**不接受 severity 降级**），
也都**不路由到补充检索**：主体归错是写错了，补充检索改不了它；
越界信息要删除或隔离，再搜只会搜到更多越界信息。
...
5. **"空"是两件事**（第六节）——检索故障与"查了没有"共用空列表，会让一次 API 超时
   变成"该企业无负面舆情"。同一条纪律在适配器层建好了，却漏了检索层这个入口，
   这是 BC-19 → BC-31 → BC-51 的第三次同形复发，可以讲"如何发现自己重复犯的错"
6. **时点隔离与回溯评测**（第七节）——判据必须是事实发布日而非取证日，
   否则机制会以"永远不给评级"的方式静默失效；而三层数据源能力不同，
   做不到的那层要明写局限而不是假装做到
...
- ~~未处理证据时效性策略（校验时间戳存在且合法，未校验是否过期）~~
  → **部分解决**：第七节的研究截止日机制已能判断证据是否**晚于**评估时点。
  仍未做的是另一个方向——证据是否**太旧**（如一年前的工商快照是否还能用）
...
### 6.1 为什么"空列表"不是一个合法的返回值

`datasource/base.py` 的 `AdapterResult` 用 `queried` / `records` 两个字段把
"没查"和"查了没有"在类型层分开，注释里写明这是 v0.2 撞出来的核心设计。

但那条纪律只落在**适配器**这一个入口上。Scout 的检索路径是另一个入口，
`_execute_search()` 有五条路径返回同一个 `[]`——四条故障加一条真空结果。

这不是"少几条搜索结果"的问题。对 `absence_meaningful=True` 的字段
（舆情、监管处罚、涉诉…），"查了没有"是**可以写进报告的正面结论**，
"没搜成"绝不是。混同两者，一次 API 超时就会以
"该企业无负面舆情"的形式进入授信依据。

详见 BC-51。**这是 BC-19 → BC-31 → BC-51 的第三次同形复发**，
每次都是"机制在别处建好了，另一个入口重新打开同一个洞"。

### 6.2 `SearchOutcome`：构造期就拒绝非法组合

```python
SearchOutcome(ok=True,  failure_reason="超时")   # ValueError
SearchOutcome(ok=False)                          # ValueError：失败必须给原因
SearchOutcome(ok=True,  results=[])              # 合法：searched_and_empty
```

`searched_and_empty` 是一个**独立的谓词**，不是 `not results`。
只有它为真时，报告才可以写"经查询未发现"。

### 6.3 失败必须走到报告里

留痕链路：`state["search_failures"]` → `search_failed` SSE →
证据附录的「本次未能完成的检索」表 → 免责声明。

结构与案例包来源目录里 S009/S010 那两条 403 记录同构：
失败被如实记为失败，并明确写"只证明本次访问未能完成查询，
不能据此认定不存在相关记录"。

**只写日志不算数**（BC-02 起反复出现的教训）：日志没人看，
而这件事直接决定报告能不能写"未发现负面记录"。

### 6.4 清单回写的判据是"查成功"，不是"有结果"

原实现是 `if all_results`，两个方向都错：

| 情形 | 原行为 | 正确行为 |
|---|---|---|
| 查成功、零结果 | 漏记 → 清单看不出这个源查过，核实率分母失真 | 记入 `attempted_sources` |
| 查失败、另一路有结果 | 记成"查过了" | 不记，另计入 `search_failures` |

另有一个三元式 bug：`tag = "web_search" if search_web else "local_kb"`，
两路都开时本地库那次检索永远不会被记录。

### 6.5 本地检索的隔离契约

范围解析与授权判断放在**持有 db 与登录身份的那一层**（router），
检索层不接受调用方传集合名——它没有做这个判断的信息
（Milvus schema 里根本没有 `user_id`）。

集合缺失时按关系库的 `document_count` 分流：登记有文档却查不到集合 →
**故障**（索引丢失或改名孤儿）；本来就没上传过 → 正常空。
这是 6.1 那条纪律在存储层的再一次应用。

### 6.6 身份一律取数据库主键（BC-53）

隔离最终失效在哪一层，取决于**身份是怎么表示的**。原实现三处都用名字派生：

| 位置 | 旧 | 问题 | 新 |
|---|---|---|---|
| 集合名 | `kb_<kb.name>` | 不唯一（只在单用户内唯一）、可改名、删除后可复用 | `kb_<UUID.hex>` |
| 切片 `kb_id` | 集合名本身 | 在集合 X 里筛 `kb_id == X`，零隔离作用 | 知识库 UUID |
| 切片 `doc_id` / 主键 | `md5(文件名)` / `md5(文件名_序号_内容)` | 同名同内容文件**主键相同，互相覆盖** | 文档 UUID / `<UUID>_<序号>` |

三条是同一个根因的三种表现：**名字面向人（可改、可重、可复用），
主键面向机器（不可变、唯一、不复用）**。把隔离建在前者上，
迟早在某个入口漏掉。

配套两条：

- **命名规则只有一处定义**（`kb_scope.collection_name_for`），
  且传知识库名直接抛 `ValueError`——不给"看起来能用但不唯一"留后路。
  让错误用法当场失败，比写一句"请传 UUID"的注释可靠
- **删除时同步清理向量**。此前 `delete_knowledge_base` 不删集合、
  `delete_document` 不删切片，在按名命名下这不只是存储泄漏：
  删掉「财报」再建同名知识库，旧向量原样复活

### 6.7 迁移的一条原则：能自动处理的自动处理，处理不了的拒绝并退出非零

旧数据里跨用户混存的集合**无法按内容拆分**——切片不携带任何可追溯到
知识库或用户的标识（`doc_id` 曾是 `md5(文件名)`）。

`scripts/migrate_kb_collections.py` 因此分两类：单一映射的逐行搬运
（带向量搬，不重新向量化，保证迁移前后向量逐位相同）；跨用户共用的
**拒绝迁移**，列出受影响知识库，要求从源文件重建。

拒绝而不是猜，是因为猜错的后果是把 A 的财报搬进 B 的知识库——比丢数据严重。
默认 dry-run，`--drop-legacy` 单独一步，让"搬完了"和"删掉了"之间留一个可回滚窗口。
...
盘点下来，时间相关机制其实很多：`retrieved_at`、
`profile_retrieved_at()` 明令禁止退化成 `now()`（BC-35）、
证据替代的单调性检查、给每个 LLM 注入的【时间基准】（BC-03）。

但它们的语义**全部是"取证时间必须真实"**，没有任何一处是
"这条证据是否晚于我们要评估的那个时点"。

后果：回溯评测做不了。钉不住时点，就只能测"现在判断对不对"，
测不了"当时判断对不对"——而后者才是信贷模型真正被考核的维度。

### 7.2 ⭐ 判据是 `as_of_date`，不是 `retrieved_at`

这是整个机制最容易搞错、且搞错后失效方式最隐蔽的一点。

| 字段 | 含义 |
|---|---|
| `retrieved_at` | **我们**什么时候取到的 |
| `as_of_date` | 这条事实什么时候**发布/发生**的 |

今天（2026-08）跑一个截止日为 2025-05-31 的案子，所有证据的
`retrieved_at` 都是今天。拿它比截止日会把**每一条**都判成越界，
整个机制退化成"设了截止日就永远不给评级"——而且没有任何报错，
只是所有报告都变成不可评级。

要卡的是另一件事：**2025-07 才披露的半年报，不得进入 2025-05-31 时点的判断**。
这正是案例包 `as_of_eligible` 的语义，也是它把 S011/S012 单独放进
`post_cutoff_outcomes` 而不是 `sources` 的原因。

单元测试里为此专门留了一条反向断言
（`test_retrieved_at_is_not_used_as_fact_date`）。

**唯一的例外是初始档案**：档案是查询时点的快照，登记状态、涉诉记录
都反映抓取当时的情形，因此对这条路径而言取证时间**就是**事实日期。
这个例外在代码里显式写明，不靠读者推断。

### 7.3 三档处理，以及为什么"日期不明"不能放行

| 情形 | 处理 | 理由 |
|---|---|---|
| 事实日期晚于截止日 | mismatch，阻断 | 用了当时不存在的信息，判断本身失效 |
| 未声明事实日期 | degradation + 专用闸门 | 不知道 ≠ 合规 |
| 事实日期早于截止日 | 通过 | — |
...
`GATE_AS_OF_UNKNOWN` 是独立的 gate kind，不并进 `GATE_PROVENANCE`。

理由是 BC-18 的教训：**闸门理由必须准确**。事实日期不明是**时点**问题，
混进"来源或取证时间不明"里报，风控人员会看到一条对不上的告警，
然后学会无视它。两类共存时各报一条，各自有独立的处置路径。

### 7.5 三层能力不同，必须分层说明

| 层 | 能否严格施加时点隔离 |
|---|---|
| L1 结构化适配器 | ✅ 有明确的字段级来源与日期 |
| L2 法定披露文件 | ✅ 有 `publication_date` |
| L3 通用网页检索 | ⚠️ **只能尽力而为** |

L3 的局限是真实存在的：检索接口返回的 `datePublished` 经常为空，
`dateLastCrawled` 是抓取时间而非发布时间，拿它当发布日会把一篇
2019 年的旧闻判成今天发的。

因此 L3 的规则是：能确认晚于截止日的**丢弃**，日期不明的
**保留并打标**，并由证据附录的「检索时点局限」一节明确写出
"未能对其施加时点过滤"。

丢弃日期不明的结果会让检索基本失效（大多数网页没有可靠发布日期）；
静默保留则是假装做到了时点隔离。**折中是保留 + 显式披露**——
这个项目里所有"做不到"的地方都按这个模式处理。

另一个决定：过滤在**客户端**做，不依赖检索接口的 `freshness` 区间语义。
正确性不该建立在未经验证的第三方参数行为上——接口若忽略或拒绝该参数，
我们会以为已经隔离了时点而实际没有。

### 7.6 提示词层的时点约束

【时间基准】（BC-03）解决的是"模型以为现在是训练截止年"。
【研究截止日】解决的是另一件事：**模型知道后来发生了什么，
于是拿它来论证当时的判断**。

后者在回溯评测里是直接的信息泄漏——用 7 月披露的半年报去证明
5 月的授信判断正确，这份评测就没有任何意义了。

约束在 `_run_agent()` **一处**统一下发给全部 Agent。
不让六个 Agent 各自在 `process()` 开头记得同步：漏掉任何一个，
那个 Agent 的提示词里就没有时点约束，**而这种遗漏在输出里看不出来**。

### 7.7 向后兼容

`as_of` 留空 = 完全不施加，行为与引入本机制之前逐字节一致。
这一点有专门的回归断言，因为绝大多数运行不需要截止日，
不能让一个为评测服务的机制影响日常路径。
