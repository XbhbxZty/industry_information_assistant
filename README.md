...
尽调与人工复核接口必须登录；复核人由后端根据 Bearer Token 自动签名，
客户端不能在请求体中指定他人身份。真实的暂停/恢复依赖 PostgreSQL，因而验收
人机协同时需要启动 Docker；纯规则、图编排替身和前端构建测试不需要。
...
- 当前版本是 **v1.0-alpha**：尽调基础页面、核实率/清单/评级展示、人工复核卡片
  和检查点恢复链路已接通；Word 导出、独立 `AuditLog`、演示交付物尚未完成。
- **RAG、向量数据库、长期记忆与 CodeWizard 没有取消。** RAG/Milvus 与记忆管理
  保留了原项目实现，但尚未完成面向尽调材料的整合验收；CodeWizard 已在 V2 图中
  执行并能产出图表，财务专用图表模板及“正文引用—图表产物”一致性仍待补齐。
  Text2SQL 仍按迁移计划延后，不属于本轮 v1.0-alpha 验收范围。

- **Critic 的检出质量尚未封板。** 裁决一致性已用规则解决，但"能不能发现问题"
  需要新的未见盲测集才能重新度量——两套盲测集都已用掉并退役为回归集。
- **数据源全部为模拟实现。** 真实适配器按同一基类接入即可，但需要付费 Key。
- 前端已新增尽调专用基础页面；担保圈可视化与报告结论到证据来源的交互式溯源尚未完成。
