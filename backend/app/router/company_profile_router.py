...
def _summary(row) -> CompanyProfileSummary:
...
def _detail(row) -> CompanyProfileResponse:
    return CompanyProfileResponse(
        **_summary(row).model_dump(), profile=row.profile_json or {},
        scenario_data=row.scenario_data or {}, field_sources=row.field_sources or [],
        materials=row.materials or [], created_at=row.created_at, created_by=row.created_by,
...
async def get_profiles(
    ...
    return CompanyProfileListResponse(items=[_summary(row) for row in rows], total=total,
                                      offset=offset, limit=limit)
...
async def get_profile(
    ...
    try:
        return _detail(get_company_profile(db, profile_id, include_archived=current_user.is_superuser))
...
@router.post("/{profile_id}/materials/search")
async def search_materials(
    profile_id: str, payload: MaterialSearchRequest,
    current_user: User = Depends(get_current_user_required), db: Session = Depends(get_db),
):
    try:
        return search_profile_materials(db, profile_id, query=payload.query, limit=payload.limit)
